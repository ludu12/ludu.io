...
export async function getCurrentlyPlaying(): Promise<{
  ...
  const data = response.data as CurrentlyPlayingObject

  if (data.item && data.currently_playing_type === 'track') {
    const item = data.item as TrackObjectFull
...
export async function getRecentlyPlayed(
  ...
  if (data && data.items) {
    const songs: SpotifySong[] = data.items.map((item: { track: TrackObjectFull }) => {
