...
import { query, toPageProperties } from './notion';
...
export async function getNotionBooks(): Promise<Book[]> {
  const result = await query(booksPageId);

  const results = result.results.map(toPageProperties) as BookColumns[];
  return results.map(r => ({
    ...r,
    cover: bookCover(r.url || ''),
  }));
}
...
//   });
//
//   return result.blockIds.map((blockId: string) => {
